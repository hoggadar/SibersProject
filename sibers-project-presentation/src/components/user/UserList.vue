...
<template>
  <div>
    ...
    <table v-if="!props.loading && props.users.length" class="bg-white border border-gray-200 shadow-md rounded-lg overflow-hidden">
    ...
    <div v-else-if="!props.loading && !props.users.length" class="mt-4 text-gray-600">Нет пользователей для отображения.</div>
  </div>
</template>
